...
header("Content-disposition:attachment;filename=Laporan_Barang_Kategori.xls");
...
if($_GET) {
	...
	if ($kodeKategori =="Semua") {
		...
		$filterSQL = " WHERE kategori_sub.kd_kategori='$kodeKategori'";
...
<table class="table-list" width="900" border="0" cellspacing="1" cellpadding="2">
  ...
  <?php
	...
	$mySql 	= "SELECT barang.*, jenis.nm_jenis FROM barang LEFT JOIN jenis ON barang.kd_jenis=jenis.kd_jenis 
				LEFT JOIN kategori_sub ON jenis.kd_kategori = kategori_sub.kd_kategori
				$filterSQL 
