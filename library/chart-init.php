<?php 
    ...
    $stringJual = "SELECT MAX(uang_bayar) AS uang_bayar FROM penjualan GROUP BY tgl_penjualan ORDER BY tgl_penjualan DESC LIMIT 0,10";
    $QueryJual = mysql_query($stringJual);
    $stringBeli = "SELECT MAX(uang_bayar) AS uang_bayar FROM pembelian_bayar 
                   GROUP BY tgl_bayar ORDER BY tgl_bayar DESC LIMIT 0,10";
    $QueryBayar = mysql_query($stringBeli);
...
jQuery(document).ready(function ($) {
    try {
        ...
        var dt1 = [];
        for (var i = 0; i <= 10; i ++) {
            dt1.push([i, parseInt(Math.random() * 10)]);
        }
        var dt2 = [];
        for (var i = 0; i <= 10; i ++) {
